...
discovery <- fread(paste("gunzip -c", args[1], sep = ' '))

##  replication:

replication <- fread(paste("gunzip -c", args[2], sep = ' '))
...
p <- ggplot(all_merged, aes(x = Discovery_OverallZScore, y = Replication_OverallZScore, colour = `Effect in replication\ncohort:`)) + 
  ...
  ggtitle(paste0('Discovery cohort: ', args[4], ' (N=', Ndisc, ')\nReplication cohort: ', args[5], ' (N=', Nrep, ')')) +
  scale_y_continuous(limits = c(-y_axis, y_axis)) + 
  scale_x_continuous(limits = c(-x_axis, x_axis))

ggsave(paste0(args[3], '_', Sys.Date(), '.png'), height = 7 * 1.2, width = 8 * 1.2, dpi = 600, units = 'in')
