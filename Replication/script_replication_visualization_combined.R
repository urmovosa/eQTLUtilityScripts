...
# file with data paths
# columns:
# Disc_path: full path to discovery dataset
# Rep_path: full path to replication analysis daset
# Disc_name: discovery dataset name occurring on the plot
# Rep_name: replication dataset name occurring on the plot

data_loc <- fread("path_file.txt")

# do plots in loop

pl <- list()

for (i in c(1:nrow(data_loc))) {

  ## read in the discovery meta-analysis results:

  discovery <- fread(paste("gunzip -c", data_loc$Disc_path[i], sep = " "))

  ##  replication:

  replication <- fread(paste("gunzip -c", data_loc$Rep_path[i], sep = " "))
  
  ## If replication dataset lacks FDR column, use this quick hack:
  ## 1. it reads in full replication results (eQTLsFDR-GeneLevel.txt.gz)
  ## 2. it replaces all FDR values with 1
  ## 3. using sig replication results (eQTLsFDR0.05-PrunedLevel.txt.gz), it replaces FDR values which are sig with 0

  if (ncol(replication) < 22){
    replication_full <- fread(paste("gunzip -c", str_replace(data_loc$Rep_path[i], 'eQTLsFDR0.05-PrunedLevel.txt.gz', 'eQTLsFDR-GeneLevel.txt.gz'), sep = ' '))
    replication_full$FDR <- 1
    replication_full$pair <- paste(replication_full$SNPName, replication_full$ProbeName, sep = '_')
    replication$pair <- paste(replication$SNPName, replication$ProbeName, sep = '_')
    replication_full[replication_full$pair %in% replication$pair, ]$FDR <- 0
    replication_full <- replication_full[, -ncol(replication_full), with = F]
    replication <- replication_full}
  ...
  if (class(Ndisc) == "matrix" & ncol(Ndisc) > 1) {
    ...
    if (class(Ndisc) == "matrix") {
      Ndisc <- max(rowSums(Ndisc))
    } else {
      (Ndisc <- sum(Ndisc))
    }
  } else {
    Ndisc <- max(as.numeric(Ndisc))
  }
  ...
  replication <- replication[, c(23, 10, 11, 22), with = F]
  colnames(replication) <- paste0("Replication_", colnames(replication))


  replication$sig_replication <- "no"
  replication[replication$Replication_FDR < 0.05, ]$sig_replication <- "yes"
  ...
  statistics_overview <- all_merged %>%
    summarize(
      nr_of_disc_assoc = length(Replication_OverallZScore),
    ...
    mutate(
      tested_eqtl_perc = round(nr_of_tested_in_replication / nr_of_disc_assoc, digits = 4) * 100,
  ...
  ###
  colnames(all_merged)[9] <- "Sig. replication"
  colnames(all_merged)[7] <- "FDR<0.05"
...
hlay <- matrix(
  nrow = length(unique(data_loc$Disc_path)),
  ncol = length(unique(data_loc$Disc_path)), 0, byrow = TRUE
)
diag(hlay) <- NA
hlay[!is.na(hlay)] <- 1:12
hlay <- t(hlay)
